...
export const fetchProductsByPage = createAsyncThunk(
  ...
  async (params: { pageIndex: number; pageSize: number }, { rejectWithValue }) => {
    try {
      const { data } = await axios.post(`${API_BASE}/api-end-user/listing/get-by-page`, {
...
export const fetchProductById = createAsyncThunk(
  ...
  async (id: string | number, { rejectWithValue }) => {
    try {
      const { data } = await axios.get(`${API_BASE}/api-end-user/listing/${id}`, {
        params: {
          aid: 'da1e0cd8-f73b-4da2-acf2-8ddc621bcf75',
        },
      });
      ...
      const ProductPrices = (item: any) => {
        ...
        const currentPrice = hasPromotion
          ? item.MinPromotionPrice ?? item.MaxPromotionPrice ?? item.MinPrice ?? item.MaxPrice ?? 0
...
export const { setListingPageIndex, clearCurrentProduct } = listingSlice.actions;
